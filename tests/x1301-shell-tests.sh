...
V4_STATE=locked1080 "$ROOT/tools/x1301/hdmi-status.sh" >"$TMP/status"; grep -qx 'STATE=LOCKED' "$TMP/status"; grep -qx 'ACTIVE_WIDTH=1920' "$TMP/status"
set +e; V4_STATE=no_signal "$ROOT/tools/x1301/hdmi-status.sh" >"$TMP/status2"; rc=$?; set -e
[[ $rc == 3 ]]; grep -qx 'power_present=1' "$TMP/status2"; grep -qx 'STATE=NO_SIGNAL' "$TMP/status2"
