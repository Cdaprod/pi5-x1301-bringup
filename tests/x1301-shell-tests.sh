...
V4_STATE=no_signal "$ROOT/tools/x1301/hdmi-status.sh" >"$TMP/status2"
grep -qx 'POWER_PRESENT=1' "$TMP/status2"; grep -qx 'SIGNAL=PRESENT_NO_SIGNAL' "$TMP/status2"
V4_STATE=disconnected "$ROOT/tools/x1301/hdmi-status.sh" >"$TMP/status3"
grep -qx 'POWER_PRESENT=0' "$TMP/status3"; grep -qx 'SIGNAL=DISCONNECTED' "$TMP/status3"
printf '%s\n' disconnected no_signal locked1080 locked1080 locked720 >"$TMP/states"; echo 1 >"$TMP/count"
V4_STATES="$TMP/states" V4_SEQUENCE_FILE="$TMP/count" HDMI_WATCH_MAX_POLLS=5 "$ROOT/tools/x1301/hdmi-watch.sh" --interval 0 >"$TMP/watch"
diff -u <(printf 'X1301 EVENT DISCONNECTED mode=none generation=0 configured=0\nX1301 EVENT PRESENT_NO_SIGNAL mode=none generation=0 configured=0\nX1301 EVENT MODE_CHANGE mode=1920x1080@59.94/148500000Hz/RGB3 generation=1 configured=0\nX1301 EVENT MODE_CHANGE mode=1280x720@60.00/74250000Hz/RGB3 generation=2 configured=0\n') "$TMP/watch"
...
python3 -m json.tool "$TMP/once" >/dev/null; grep -q '"event":"MODE_CHANGE"' "$TMP/once"
